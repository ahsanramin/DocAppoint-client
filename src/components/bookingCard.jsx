...
const BookingCard = ({ booking, bookings, setBookings }) => {
    ...
    return (
        <div className="bg-white border border-blue-200/60 p-5 rounded-xl shadow-md space-y-4 max-w-sm w-full">
            {/* Doctor Name */}
            <h3 className="font-semibold text-blue-800 dark:text-blue-400 text-xl tracking-tight">
                {booking.doctorName}
            </h3>
            {/* Details Area - Showing Patient, Gender, Date, Time */}
            <div className="space-y-2.5 text-sm text-slate-600 dark:text-slate-400 border-l-2 border-blue-200 pl-3">
            ...
            <div className="flex items-center justify-start gap-2.5 pt-1">
                <button onClick={() => setEditingBooking(booking)}
                    className="flex items-center gap-1 px-3.5 py-1.5 text-white bg-blue-500  rounded-lg text-xs font-medium hover:bg-blue-700  dark:text-slate-300 transition-colors cursor-pointer">
                 <Pencil className="h-3.5 w-3.5 b" /> Update</button>
                <AlertDialog>
                    <Button size="sm" color="danger" variant="solid" className="flex items-center gap-1 px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-medium transition-colors cursor-pointer">
                    ...
                    <AlertDialog.Backdrop>
                        <AlertDialog.Container>
                            <AlertDialog.Dialog className="sm:max-w-100">
                                ...
                                <AlertDialog.Body>
                                    <p>Are you sure you want to cancel your appointment with <strong>{booking.doctorName}</strong>? This action cannot be undone.</p>
                                </AlertDialog.Body>
                                <AlertDialog.Footer>
                                    <Button slot="close" variant="flat">Cancel</Button>
                                    <Button slot="close" color="danger" onClick={() => handleDelete(booking._id)} className="bg-rose-500" >
                                        Yes, Delete
                                    </Button>
                                </AlertDialog.Footer>
                            </AlertDialog.Dialog>
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl max-w-md w-full p-6 space-y-4 relative shadow-2xl">
                       ...
                       <X className="h-5 w-5" />
                        ...
                        <form onSubmit={handleUpdateSubmit} className="space-y-4 text-sm">
                            ...
                            <button
                                type="submit"
                                className="w-full py-2.5 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-all cursor-pointer shadow-sm">
                                Save Changes
                            </button>
                        </form>
                    </div>
